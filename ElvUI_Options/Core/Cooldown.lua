...
local function group(order, db, label)
	local main = ACH:Group(
		...
		nil,
		function()
			return db == "WeakAuras" and not IsAddOnLoaded(db)
		end
	)
	E.Options.args.cooldown.args[db] = main

	local mainArgs = main.args
	mainArgs.reverse = ACH:Toggle(
		L["Reverse Toggle"],
	...
	mainArgs.hideBlizzard = ACH:Toggle(
		L["Force Hide Blizzard Text"],
		...
		function()
			if db == "global" then
				...
				return (E.db.cooldown.enable and not profile(db).reverse)
					or (not E.db.cooldown.enable and profile(db).reverse)
			end
		end
	)

	local seconds = ACH:Group(L["Text Threshold"], nil, 10, nil, function(info)
		return (profile(db))[info[#info]]
	...
	seconds.inline = true
	seconds.args.checkSeconds = ACH:Toggle(
		L["Enable"],
	...
	seconds.args.mmssThreshold = ACH:Range(
		L["MM:SS Threshold"],
	...
	seconds.args.hhmmThreshold = ACH:Range(
		L["HH:MM Threshold"],
	...
	local fonts = ACH:Group(L["Fonts"], nil, 11, nil, function(info)
	...
	local colors = ACH:Group(L["Color Override"], nil, 12, nil, nil, nil, function()
		return not (profile(db)).override
	end)
	colors.inline = true
	colors.args.override = ACH:Toggle(
		L["Enable"],
		L["This will override the global cooldown settings."],
		1,
		nil,
	...
	colors.args.threshold = ACH:Range(
		L["Low Threshold"],
	...
	tColors.args.modRateColor = ACH:Color(
		L["Modified Rate"],
	...
	local iColors = ACH:Group(L["Time Indicator Colors"], nil, 4, nil, nil, nil, function()
		return not (profile(db)).useIndicatorColor
	end)
	iColors.args.useIndicatorColor = ACH:Toggle(L["Use Indicator Color"], nil, 0, nil, nil, nil, function(info)
		return (profile(db))[info[#info]]
	end, function(info, value)
		(profile(db))[info[#info]] = value
		E:UpdateCooldownSettings(db)
	...
	if db == "global" then
		...
		mainArgs.roundTime = ACH:Toggle(L["Round Timers"], nil, 1, nil, nil, nil, function(info)
			return (profile(db))[info[#info]]
		end, function(info, value)
			(profile(db))[info[#info]] = value
			E:UpdateCooldownSettings(db)
	...
	elseif db == "actionbar" then
		...
		auraGroup.args.targetAura = ACH:Toggle(
			L["Enable"],
		...
		auraGroup.args.targetAuraDuration = ACH:Range(
			L["Maximum Duration"],
			...
			function(info, value)
				E.db.cooldown[info[#info]] = value
				if AB.Initialized then
					AB:SetAuraCooldownDuration(value)
				end
			end
		)
		...
		auraGroup.args.expiringAuraColor =
			ACH:Color(L["Target Aura Expiring"], L["Color of the Targets Aura time when expiring."], 7)
		auraGroup.args.expiringAuraColor.customWidth = 175

		auraGroup.args.targetAuraIndicator =
			ACH:Color(L["Indicator"], L["Color of the Targets Aura time."], 10, nil, nil, nil, nil, false)
		auraGroup.args.targetAuraIndicator.customWidth = 125
		auraGroup.args.expiringAuraIndicator = ACH:Color(
			L["Indicator Expiring"],
...
E.Options.args.cooldown = ACH:Group(L["Cooldown Text"], nil, 2, "tab", function(info)
	return E.db.cooldown[info[#info]]
end, function(info, value)
	E.db.cooldown[info[#info]] = value
...
E.Options.args.cooldown.args.enable =
	ACH:Toggle(L["Enable"], L["Display cooldown text on anything with the cooldown spiral."], 1)
