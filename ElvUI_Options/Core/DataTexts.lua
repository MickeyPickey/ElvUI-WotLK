...
DTPanelOptions.textJustify = ACH:Select(
	L["Text Justify"],
	L["Sets the font instance's horizontal text alignment style."],
	6,
	{ CENTER = L["Center"], LEFT = L["Left"], RIGHT = L["Right"] }
)

DTPanelOptions.templateGroup = ACH:MultiSelect(L["Template"], nil, 10, {
	backdrop = L["Backdrop"],
...
DTPanelOptions.tooltip.args.tooltipAnchor = ACH:Select(L["Anchor"], nil, 0, {
	ANCHOR_TOP = L["TOP"],
...
DTPanelOptions.tooltip.args.tooltipXOffset =
	ACH:Range(L["X-Offset"], nil, 1, { softMin = -30, softMax = 30, min = -60, max = 60, step = 1 })
DTPanelOptions.tooltip.args.tooltipYOffset =
	ACH:Range(L["Y-Offset"], nil, 2, { softMin = -30, softMax = 30, min = -60, max = 60, step = 1 })

DTPanelOptions.visibility = ACH:Input(
	L["Visibility State"],
...
function DT:SetupPanelOptions(name, data)
	...
	if not options then
		options = ACH:Group(ColorizeName(name, not custom and "ffffff"), nil, nil, nil, function(info)
			return db[info[#info]]
		...
		if custom then
			...
			options.args.panelOptions = ACH:Group(L["Panel Options"], nil, 5, nil, function(info)
			...
			options.args.panelOptions.args.delete = ACH:Execute(L["Delete"], nil, -1, function()
				PanelGroup_Delete(name)
			end, nil, true, "full")

			options.args.panelOptions.args.fonts = ACH:Group(L["Fonts"], nil, 10, nil, function(info)
				return custom.fonts[info[#info]]
			end, function(info, value)
				custom.fonts[info[#info]] = value
				DT:UpdatePanelAttributes(name, custom)
			end, function()
				return not custom.fonts.enable
			end)
			options.args.panelOptions.args.fonts.args.enable =
				ACH:Toggle(L["Enable"], nil, 0, nil, nil, nil, nil, nil, false)
			options.args.panelOptions.args.fonts.args.font = ACH:SharedMediaFont(L["Font"], nil, 1)
			options.args.panelOptions.args.fonts.args.fontOutline =
				ACH:FontFlags(L["Font Outline"], L["Set the font outline."], 2)
			...
			panelOpts.tooltip.args.tooltipYOffset.disabled = function()
				return custom.tooltipAnchor == "ANCHOR_CURSOR"
			end
			panelOpts.tooltip.args.tooltipXOffset.disabled = function()
				return custom.tooltipAnchor == "ANCHOR_CURSOR"
			end
			...
			panelOpts.templateGroup.get = function(_, key)
				return custom[key]
			end
		...
		elseif not (P.datatexts.panels[name] or custom) then
			options.args.delete = ACH:Execute(L["Delete"], nil, 2, function()
				PanelGroup_Delete(name)
			end)
			return
		end
	...
	for i = 1, DTPanelOptions.numPoints.softMax do
		if not options.args.dts then
			options.args.dts = ACH:Group(" ", nil, 3, nil, function(info)
				return db[tonumber(info[#info])] or ""
			end, function(info, value)
				db[tonumber(info[#info])] = value
				DT:UpdatePanelInfo(name)
			end)
		...
		if data and data.battleground ~= nil then
			...
			if not options.args.battledts then
				options.args.battledts = ACH:Group(
					L["Battlegrounds"],
					nil,
					4,
					nil,
					function(info)
						return E.db.datatexts.battlePanel[name][tonumber(info[#info])]
					end,
					function(info, value)
						E.db.datatexts.battlePanel[name][tonumber(info[#info])] = value
						DT:UpdatePanelInfo(name)
					end,
					nil,
					function()
						return not data.battleground
					end
				)
...
local function CreateDTOptions(name, data)
	...
	if data.isCurrency then
		...
		optionTable = ACH:Group(format("%s |cFF888888[%s]|r", currency.name, name), nil, 1, nil, function(info)
			return E.global.datatexts.customCurrencies[name][info[#info]]
		end, function(info, value)
			E.global.datatexts.customCurrencies[name][info[#info]] = value
			DT:LoadDataTexts()
		end)

		optionTable.args.nameStyle =
			ACH:Select(L["Name Style"], nil, 1, { full = L["Name"], abbr = L["Abbreviate Name"], none = L["None"] })
		optionTable.args.showIcon = ACH:Toggle(L["Show Icon"], nil, 2)
		optionTable.args.showMax = ACH:Toggle(L["Current / Max"], nil, 3)
		optionTable.args.currencyTooltip = ACH:Toggle(
			L["Display In Main Tooltip"],
			...
			nil,
			function()
				return DT.CurrencyList[tostring(name)]
			end
		)
		...
		if not settings or (settings and not next(settings)) then
			return
		end

		optionTable = ACH:Group(data.localizedName or name, nil, nil, nil, function(info)
			return settings[info[#info]]
		end, function(info, value)
			settings[info[#info]] = value
			DT:ForceUpdate_DataText(name)
		end)
		E.Options.args.datatexts.args.settings.args[name] = optionTable
	end

	if data.isLDB then
		optionTable.args.customLabel = ACH:Input(L["Custom Label"], nil, 1, nil, nil, function(info)
			return escapeString(settings[info[#info]], true)
		end, function(info, value)
			settings[info[#info]] = escapeString(value)
			DT:LoadDataTexts()
		end)
		...
		for key in pairs(settings) do
			if key == "Label" then
				optionTable.args[key] = ACH:Input(L["Label"], nil, 2, nil, nil, function(info)
					return escapeString(settings[info[#info]], true)
				end, function(info, value)
					settings[info[#info]] = escapeString(value)
					DT:ForceUpdate_DataText(name)
			...
			elseif key == "goldFormat" then
				optionTable.args[key] = ACH:Select(
					L["Gold Format"],
			...
			elseif key == "textFormat" then
				optionTable.args[key] = ACH:Select(L["Text Format"], nil, 20, nil, nil, "double", function(info)
					return settings[info[#info]]
				end, function(info, value)
					settings[info[#info]] = value
					DT:ForceUpdate_DataText(name)
			...
			elseif key == "latency" then
				optionTable.args[key] =
					ACH:Select(L["Latency"], nil, 20, { WORLD = L["World Latency"], HOME = L["Home Latency"] })
			elseif key == "school" then
				optionTable.args[key] = ACH:Select(L["School"], nil, 20, {
					[0] = "Default",
					[1] = "Physical",
					[2] = "Holy",
					[3] = "Fire",
		...
		elseif name == "CombatIndicator" then
			optionTable.args.OutOfCombat = ACH:Input(L["Out of Combat Label"], nil, 1, nil, nil, function(info)
				return escapeString(settings[info[#info]], true)
			end, function(info, value)
				settings[info[#info]] = escapeString(value)
				DT:ForceUpdate_DataText(name)
			end)
			optionTable.args.OutOfCombatColor = ACH:Color("", nil, 2, nil, nil, function(info)
				local c, d = settings[info[#info]], G.datatexts.settings[name][info[#info]]
			...
			optionTable.args.InCombat = ACH:Input(L["In Combat Label"], nil, 4, nil, nil, function(info)
				return escapeString(settings[info[#info]], true)
			end, function(info, value)
				settings[info[#info]] = escapeString(value)
				DT:ForceUpdate_DataText(name)
			end)
			optionTable.args.InCombatColor = ACH:Color("", nil, 5, nil, nil, function(info)
				local c, d = settings[info[#info]], G.datatexts.settings[name][info[#info]]
				return c.r, c.g, c.b, nil, d.r, d.g, d.b
			end, function(info, r, g, b)
				local c = settings[info[#info]]
		...
		elseif name == "Currencies" then
			optionTable.args.displayedCurrency = ACH:Select(L["Displayed Currency"], nil, 10, function()
				...
				for _, info in pairs(E.global.datatexts.customCurrencies) do
					...
					if info and not DT.CurrencyList[id] then
						list[id] = info.name
					end
				end
				return list
			...
			optionTable.args.displayStyle = ACH:Select(
				L["Display Style"],
				nil,
				1,
				{
				...
				nil,
				function()
					return (settings.displayedCurrency == "GOLD") or (settings.displayedCurrency == "BACKPACK")
				end
			)
			...
			for i, info in ipairs(G.datatexts.settings.Currencies.tooltipData) do
				...
				elseif info[3] then
					optionTable.args.tooltipLines.args[tostring(info[3])].args[tostring(i)] = ACH:Toggle(
						info[1],
						nil,
						i,
						nil,
						...
						nil,
						function()
							return settings.idEnable[info[2]]
						end,
						function(_, value)
							settings.idEnable[info[2]] = value
						end
					)
				end
			end
		elseif name == "Durability" then
			optionTable.args.percThreshold = ACH:Range(
				L["Flash Threshold"],
		...
		elseif name == "Friends" then
			...
			optionTable.args.hideGroup1 = ACH:MultiSelect(
				L["Hide by Status"],
				nil,
				5,
				{ hideAFK = L["AFK"], hideDND = L["DND"] },
				nil,
				nil,
				function(_, key)
					return settings[key]
			...
			optionTable.args.hideGroup2 = ACH:MultiSelect(
				L["Hide by Application"],
				nil,
				6,
				DT.clientFullName,
				nil,
				nil,
				function(_, key)
					return settings["hide" .. key]
				end,
				function(_, key, value)
					settings["hide" .. key] = value
					DT:ForceUpdate_DataText(name)
				end
			)
		...
		elseif name == "Location" then
			...
			optionTable.args.color = ACH:Select(
				L["Text Color"],
				nil,
				10,
				{ REACTION = L["Reaction"], CLASS = L["CLASS"], CUSTOM = L["CUSTOM"] }
			)
			optionTable.args.customColor = ACH:Color("", nil, 11, nil, nil, function(info)
				local c, d = settings[info[#info]], G.datatexts.settings[name][info[#info]]
				return c.r, c.g, c.b, nil, d.r, d.g, d.b
			end, function(info, r, g, b)
				local c = settings[info[#info]]
		...
		elseif name == "Talent Specialization" then
			optionTable.args.iconOnly =
				ACH:Toggle(L["Icons Only"], L["Only show icons instead of specialization names"], 2)
		...
		elseif name == "Time" then
			...
			optionTable.args.localTime =
				ACH:Toggle(L["Local Time"], L["If not set to true then the server time will be displayed instead."], 7)
			optionTable.args.flashInvite = ACH:Toggle(
				L["Flash Invites"],
...
defaultTemplateGroup.args.border = ACH:Toggle(L["Border"], nil, 2, nil, nil, nil, nil, nil, function(info)
	return not E.db.datatexts.panels[info[#info - 2]].backdrop
end)
defaultTemplateGroup.args.panelTransparency = ACH:Toggle(
	L["Panel Transparency"],
	nil,
	3,
	nil,
	nil,
...
local DataTexts = ACH:Group(L["DataTexts"], nil, 2, "tab", function(info)
...
DataTexts.args.general.args.generalGroup.args.noCombatClick =
	ACH:Toggle(L["Block Combat Click"], L["Blocks all click events while in combat."], 2)
DataTexts.args.general.args.generalGroup.args.noCombatHover =
	ACH:Toggle(L["Block Combat Hover"], L["Blocks datatext tooltip from showing in combat."], 3)
...
DataTexts.args.panels.args.newPanel = ACH:Group(ColorizeName(L["New Panel"], "33ff33"), nil, 0, nil, function(info)
	return E.global.datatexts.newPanelInfo[info[#info]]
end, function(info, value)
	E.global.datatexts.newPanelInfo[info[#info]] = value
end)
DataTexts.args.panels.args.newPanel.args.name = ACH:Input(
	L["Name"],
	nil,
	0,
	nil,
...
DataTexts.args.panels.args.newPanel.args.add = ACH:Execute(
	L["Add"],
	nil,
	1,
	function()
		...
		E.db.datatexts.panels[name] = { enable = true, battleground = false }
...
DataTexts.args.panels.args.newPanel.args.templateGroup.get = function(_, key)
	return E.global.datatexts.newPanelInfo[key]
end
DataTexts.args.panels.args.newPanel.args.templateGroup.set = function(_, key, value)
	E.global.datatexts.newPanelInfo[key] = value
end

DataTexts.args.panels.args.LeftChatDataPanel = ACH:Group(
	ColorizeName(L["Datatext Panel (Left)"], "cccccc"),
	L["Display data panels below the chat, used for datatexts."],
	1,
	nil,
	function(info)
		return E.db.datatexts.panels.LeftChatDataPanel[info[#info]]
	end,
	function(info, value)
		E.db.datatexts.panels.LeftChatDataPanel[info[#info]] = value
		DT:UpdatePanelInfo("LeftChatDataPanel")
		LO:SetDataPanelStyle()
	end
)
DataTexts.args.panels.args.LeftChatDataPanel.args.enable = ACH:Toggle(
	L["Enable"],
	nil,
	0,
	nil,
	...
	function(info, value)
		...
		LO:ToggleChatPanels()
		LO:SetDataPanelStyle()
		DT:UpdatePanelInfo(info[#info - 1])
	end
)
DataTexts.args.panels.args.LeftChatDataPanel.args.templateGroup = CopyTable(defaultTemplateGroup)

DataTexts.args.panels.args.RightChatDataPanel = ACH:Group(
	ColorizeName(L["Datatext Panel (Right)"], "cccccc"),
	L["Display data panels below the chat, used for datatexts."],
	1,
	nil,
	function(info)
		return E.db.datatexts.panels.RightChatDataPanel[info[#info]]
	end,
	function(info, value)
		E.db.datatexts.panels.RightChatDataPanel[info[#info]] = value
		DT:UpdatePanelInfo("RightChatDataPanel")
		LO:SetDataPanelStyle()
	end
)
DataTexts.args.panels.args.RightChatDataPanel.args.enable = ACH:Toggle(
	L["Enable"],
	nil,
	0,
	nil,
...
DataTexts.args.panels.args.MinimapPanel = ACH:Group(
	ColorizeName(L["Minimap Panels"], "cccccc"),
	L["Display minimap panels below the minimap, used for datatexts."],
	3,
	nil,
	function(info)
		return E.db.datatexts.panels.MinimapPanel[info[#info]]
	end,
	function(info, value)
		E.db.datatexts.panels.MinimapPanel[info[#info]] = value
...
DataTexts.args.panels.args.MinimapPanel.args.enable = ACH:Toggle(
	L["Enable"],
	nil,
	0,
	nil,
...
DataTexts.args.panels.args.MinimapPanel.args.numPoints = ACH:Range(
	L["Number of DataTexts"],
	nil,
	2,
	{ min = 1, max = 2, step = 1 },
	nil,
	...
	function(info, value)
		E.db.datatexts.panels.MinimapPanel[info[#info]] = value
		DT:UpdatePanelInfo("MinimapPanel")
		DT:SetupPanelOptions("MinimapPanel")
	end
)
...
DataTexts.args.customCurrency.args.description = ACH:Description(
...
DataTexts.args.customCurrency.args.add =
	ACH:Select(L["Add Currency"], nil, 1, getCurrencyList, nil, "double", nil, addCurrency)
DataTexts.args.customCurrency.args.addID =
	ACH:Input(L["Add Currency by ID"], nil, 2, nil, "double", C.Blank, addCurrency)
DataTexts.args.customCurrency.args.delete = ACH:Select(
	L["Delete"],
	nil,
	2,
	function()
		...
		for currencyID, info in pairs(E.global.datatexts.customCurrencies) do
			currencyList[currencyID] = info.name
		end
		return currencyList
	end,
	nil,
	...
	function()
		return not next(E.global.datatexts.customCurrencies)
	end
)
