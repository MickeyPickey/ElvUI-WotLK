...
E.Options.args.tooltip = ACH:Group(L["Tooltip"], nil, 2, "tab", function(info)
...
Tooltip.enable = ACH:Toggle(L["Enable"], nil, 2, nil, nil, nil, function(info)
...
General.inspectDataEnable = ACH:Toggle(
	L["Inspect Data"],
...
General.colorAlpha = ACH:Range(
	L["OPACITY"],
	nil,
	11,
	{ isPercent = true, min = 0, max = 1, step = 0.01 },
	nil,
	nil,
	function(info, value)
		E.db.tooltip[info[#info]] = value
...
General.modifierGroup.args.modifierCount = ACH:Toggle(
	L["Modifier Count"],
	...
	nil,
	function()
		return E.db.tooltip.itemCount == "NONE"
	end
)
General.modifierGroup.inline = true

General.itemCount = ACH:MultiSelect(
	L["Item Count"],
	L["Display how many of a certain item you have in your possession."],
	-2,
	{ bags = L["Bags"], stack = L["Stack Size"] },
	nil,
	nil,
	function(_, key)
		return E.db.tooltip.itemCount[key]
	end,
	function(_, key, value)
		E.db.tooltip.itemCount[key] = value
	end
)
...
General.anchorGroup.args.cursorAnchorType = ACH:Select(
	L["Cursor Anchor Type"],
	nil,
	6,
	{ ANCHOR_CURSOR = L["CURSOR"], ANCHOR_CURSOR_LEFT = L["CURSOR_LEFT"], ANCHOR_CURSOR_RIGHT = L["CURSOR_RIGHT"] },
	nil,
	nil,
	nil,
	nil,
...
General.anchorGroup.args.cursorAnchorX = ACH:Range(
...
General.anchorGroup.args.cursorAnchorY = ACH:Range(
...
General.factionColors = ACH:Group(L["Custom Faction Colors"], nil, 60, nil, function(info)
...
General.factionColors.args.useCustomFactionColors = ACH:Toggle(L["Enable"], nil, 0, nil, nil, nil, function()
...
for i = 1, 8 do
	General.factionColors.args["" .. i] = ACH:Color(
		L["FACTION_STANDING_LABEL" .. i],
		nil,
		i,
		true,
		nil,
...
General.fontGroup = ACH:Group(L["Font"], nil, 70, nil, function(info)
	return E.db.tooltip[info[#info]]
end, function(info, value)
	E.db.tooltip[info[#info]] = value
	TT:SetTooltipFonts()
end)
General.fontGroup.args.smallTextFontSize = ACH:Range(
	L["Comparison Font Size"],
...
General.healthBar = ACH:Group(L["Health Bar"], nil, 80, nil, function(info)
	return E.db.tooltip.healthBar[info[#info]]
end, function(info, value)
	E.db.tooltip.healthBar[info[#info]] = value
end)
General.healthBar.args.statusPosition =
	ACH:Select(L["Position"], nil, 1, { BOTTOM = L["Bottom"], TOP = L["Top"], DISABLED = L["Disabled"] })
General.healthBar.args.height = ACH:Range(
	L["Height"],
	nil,
	3,
	{ min = 2, max = 15, step = 1 },
	...
	function()
		return E.db.tooltip.healthBar.statusPosition == "DISABLED"
	end
)
General.healthBar.args.text = ACH:Toggle(L["Text"], nil, 3, nil, nil, nil, nil, function(_, value)
...
General.healthBar.args.font = ACH:SharedMediaFont(L["Font"], nil, 4, nil, nil, function(_, value)
	...
	return not E.db.tooltip.healthBar.text or E.db.tooltip.healthBar.statusPosition == "DISABLED"
end)
General.healthBar.args.fontSize = ACH:Range(L["Font Size"], nil, 5, C.Values.FontSize, nil, nil, function(_, value)
...
General.healthBar.args.fontOutline = ACH:FontFlags(L["Font Outline"], nil, 6, nil, nil, function(_, value)
...
General.visibility = ACH:Group(L["Visibility"], nil, 90, nil, function(info)
	return E.db.tooltip.visibility[info[#info]]
end, function(info, value)
	E.db.tooltip.visibility[info[#info]] = value
end)
General.visibility.args.actionbars = ACH:Select(
	L["ActionBars"],
	L["Choose when you want the tooltip to show. If a modifier is chosen, then you need to hold that down to show the tooltip."],
	1,
	modifierValues
)
General.visibility.args.bags = ACH:Select(
	L["Bags/Bank"],
	L["Choose when you want the tooltip to show. If a modifier is chosen, then you need to hold that down to show the tooltip."],
	2,
	modifierValues
)
General.visibility.args.unitFrames = ACH:Select(
	L["UnitFrames"],
	L["Choose when you want the tooltip to show. If a modifier is chosen, then you need to hold that down to show the tooltip."],
	3,
	modifierValues
)
General.visibility.args.combatOverride = ACH:Select(
