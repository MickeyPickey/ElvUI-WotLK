...
do
	C.StateSwitchGetText = function(_, TEXT)
		...
		return (friend and format("|cFF33FF33%s|r %s", L["Friend"], filterText))
			or (enemy and format("|cFFFF3333%s|r %s", ENEMY, filterText))
			or filterText
	end
...
local DEVELOPERS = {
	...
	E:TextureString([[Interface\Icons\Inv_Misc_Monsterclaw_04]], ":21:21:0:0:64:64:5:59:5:59")
...
local TESTERS = {
	...
	"Caedis",
	"|cff00c0faBenik|r",
	E:TextureString([[Interface\Icons\Spell_Nature_Bloodlust]], ":21:21:0:0:64:64:5:59:5:59")
		.. " |cff006fdcRubgrsch|r",
	...
	E:TextureString([[Interface\Icons\Inv_Misc_Monsterclaw_04]], ":21:21:0:0:64:64:5:59:5:59")
		.. " |cffFF7D0ABladesdruid|r - AKA SUPERBEAR",
}
...
E.Options.args.info.args.debug = ACH:Execute(L["Debug"], L["DEBUG_DESC"], 1, function()
	local state = next(ElvDB.DisabledAddOns)
	E:LuaError(state and "off" or "on")
end, nil, nil, 120)
E.Options.args.info.args.colors = ACH:Execute(L["Color Picker"], nil, 2, function()
	_G.ColorPickerFrame:Show()
	_G.ColorPickerFrame:SetFrameStrata("FULLSCREEN_DIALOG")
	_G.ColorPickerFrame:SetClampedToScreen(true)
	_G.ColorPickerFrame:Raise()
end, nil, nil, 120)
...
for index, data in
	next,
	{
		...
		{
			key = "master",
			name = L["Master Version"],
			url = "https://github.com/ElvUI-WotLK/ElvUI/archive/refs/heads/master.zip",
		},
		{
			key = "dev",
			name = L["Development Version"],
			url = "https://github.com/ElvUI-WotLK/ElvUI/archive/refs/heads/development.zip",
		},
		{
			key = "changelog",
			name = L["Changelog"],
			url = "https://github.com/ElvUI-WotLK/ElvUI/blob/development/CHANGELOG.md",
		},
		{
			key = "customTexts",
			name = L["Custom Texts"],
			url = "https://github.com/ElvUI-WotLK/ElvUI/wiki/custom-texts",
		},
		{ key = "paging", name = L["Action Paging"], url = "https://github.com/ElvUI-WotLK/ElvUI/wiki/paging" },
		{
			key = "performance",
			name = L["Performance"],
			url = "https://github.com/ElvUI-WotLK/ElvUI/wiki/performance-optimization",
		},
	}
do
	E.Options.args.info.args.main.args[data.key] = ACH:Input(data.name, nil, index, nil, 255, function()
		return data.url
	end)
	E.Options.args.info.args.main.args[data.key].focusSelect = true
end

local credits = ("*%s|r|cFFffffff below.  Made with|r |cFFff75dd<3|r |cFFffffffby the Tukui Community.|r"):gsub(
	"*",
	E.InfoColor
)
...
E.Options.args.profiles.args.desc =
	ACH:Description(L["This feature will allow you to transfer settings to other characters."], 0)
E.Options.args.profiles.args.distributeProfile = ACH:Execute(
	L["Share Profile"],
	...
	nil,
	function()
		return not E.global.general.allowDistributor
	end
)
E.Options.args.profiles.args.distributePrivate = ACH:Execute(
	L["Share Private"],
	nil,
	2,
	function()
		...
		local name, server = UnitName("target")
		if name and (not server or server == "") then
			D:Distribute(name, false, "private")
		elseif server then
			D:Distribute(name, true, "private")
		end
	end,
	nil,
	nil,
...
E.Options.args.profiles.args.distributeGlobal = ACH:Execute(
	L["Share Global"],
	nil,
	3,
	function()
		...
		local name, server = UnitName("target")
		if name and (not server or server == "") then
			D:Distribute(name, false, "global")
		elseif server then
			D:Distribute(name, true, "global")
		end
	end,
	nil,
	nil,
...
E.Options.args.profiles.args.allowDistributor = ACH:Toggle(
	L["Allow Sharing"],
...
do -- Import and Export
	...
	do
		...
		local function Import(which)
			...
			for _ in next, profileTypeItems do
				...
				if which == "text" and validateString(nil, importText) then
					...
					label.name = (imported and L["Profile imported successfully!"])
						or L["Error decoding data. Import string may be corrupted!"]
				else
		...
		local function Import_Get(info)
			return importTexts[info[#info]]
		end
		BuildEditboxes(import, Import_Get, Import_Set, false, importTexts)

		import.args.importButton = ACH:Execute(L["Import"], nil, 1, function()
			Import("text")
		end, nil, nil, 120)
		import.args.decodeButton = ACH:Execute(L["Decode"], nil, 2, function()
			Import("luaTable")
		end, nil, nil, 120)
		import.args.clearButton = ACH:Execute(L["Clear"], nil, 3, function()
			Import("clear")
		end, nil, nil, 120)
	end
	...
	do
		...
		local function Export_Get(info)
			return exportTexts[info[#info]]
		end
		...
		local function Filters_Empty()
			return not next(exportList)
		end
		...
		local function Filters_Set(_, key, value)
			exportList[key] = value or nil
		end
		...
		export.args.exportButton = ACH:Execute(L["Export"], nil, 1, function()
			Export("text")
		end, nil, nil, 120)
		export.args.decodeButton = ACH:Execute(L["Table"], nil, 2, function()
			Export("luaTable")
		end, nil, nil, 120)
		export.args.pluginButton = ACH:Execute(L["Plugin"], nil, 3, function()
			Export("luaPlugin")
		end, nil, nil, 120)
		export.args.profileTye =
			ACH:MultiSelect(L["Choose What To Export"], nil, 10, profileTypeItems, nil, nil, Filters_Get, Filters_Set)
...
do -- Module Copy
	...
	local function CreateNamePlatesConfig()
		...
		config.args.units = ACH:Group(L["Nameplates"], nil, -10, nil, function(info)
	...
	local function CreateUnitframesConfig()
		...
		config.args.cooldown = ACH:Toggle(L["Cooldown Text"], nil, 2, nil, nil, nil, function(info)
			return E.global.profileCopy.unitframe[info[#info]]
		end, function(info, value)
			E.global.profileCopy.unitframe[info[#info]] = value
		end)
		config.args.colors = ACH:Group(L["Colors"], nil, -9, nil, function(info)
		...
		config.args.units = ACH:Group(L["UnitFrames"], nil, -10, nil, function(info)
			return E.global.profileCopy.unitframe[info[#info - 1]][info[#info]]
		end, function(info, value)
			E.global.profileCopy.unitframe[info[#info - 1]][info[#info]] = value
		end)
	...
	E.Options.args.profiles.args.modulecopy.args.elvui =
		ACH:Group("ElvUI", L["Core |cff1784d1ElvUI|r options."], 10, "tree")
	...
	E.Options.args.profiles.args.modulecopy.args.movers =
		ACH:Group(L["Movers"], L["On screen positions for different elements."], 20, "tree")
	E.Options.args.profiles.args.modulecopy.args.movers.args = MC:CreateMoversConfigGroup()

	E.Options.args.profiles.args.modulereset = ACH:Group(
		L["Module Reset"],
		nil,
		6,
		"tab",
		...
		nil,
		function(info)
			E:CopyTable(E.db[info[#info]], P[info[#info]])
		end
	)
	E.Options.args.profiles.args.modulereset.args.intro =
		ACH:Description(L["This section will help reset specfic settings back to default."], 1)
	E.Options.args.profiles.args.modulereset.args.space1 = ACH:Spacer(2)
	E.Options.args.profiles.args.modulereset.args.general =
		ACH:Execute(L["General"], nil, 3, nil, nil, L["Are you sure you want to reset General settings?"])
	E.Options.args.profiles.args.modulereset.args.actionbar =
		ACH:Execute(L["ActionBars"], nil, 4, nil, nil, L["Are you sure you want to reset ActionBars settings?"])
	E.Options.args.profiles.args.modulereset.args.bags =
		ACH:Execute(L["Bags"], nil, 5, nil, nil, L["Are you sure you want to reset Bags settings?"])
	E.Options.args.profiles.args.modulereset.args.auras =
		ACH:Execute(L["Auras"], nil, 6, nil, nil, L["Are you sure you want to reset Auras settings?"])
	E.Options.args.profiles.args.modulereset.args.chat =
		ACH:Execute(L["Chat"], nil, 7, nil, nil, L["Are you sure you want to reset Chat settings?"])
	E.Options.args.profiles.args.modulereset.args.cooldown =
		ACH:Execute(L["Cooldown Text"], nil, 8, nil, nil, L["Are you sure you want to reset Cooldown settings?"])
	E.Options.args.profiles.args.modulereset.args.databars =
		ACH:Execute(L["DataBars"], nil, 9, nil, nil, L["Are you sure you want to reset DataBars settings?"])
	E.Options.args.profiles.args.modulereset.args.datatexts =
		ACH:Execute(L["DataTexts"], nil, 10, nil, nil, L["Are you sure you want to reset DataTexts settings?"])
	E.Options.args.profiles.args.modulereset.args.nameplates =
		ACH:Execute(L["Nameplates"], nil, 11, nil, nil, L["Are you sure you want to reset NamePlates settings?"])
	E.Options.args.profiles.args.modulereset.args.tooltip =
		ACH:Execute(L["Tooltip"], nil, 12, nil, nil, L["Are you sure you want to reset Tooltip settings?"])
	E.Options.args.profiles.args.modulereset.args.uniframes = ACH:Execute(L["UnitFrames"], nil, 13, function()
		E:CopyTable(E.db.unitframe, P.unitframe)
		UF:Update_AllFrames()
	end, nil, L["Are you sure you want to reset UnitFrames settings?"])
end
