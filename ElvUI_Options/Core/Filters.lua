...
local defaultFilterList = {
	...
	["AuraBar Colors"] = "AuraBar Colors" --[[ , ['Aura Highlight'] = 'Aura Highlight' ]],
}
...
local function GetSelectedFilters()
	...
	local selected = (profile and E.db.unitframe.filters.buffwatch)
		or (pet and E.global.unitframe.buffwatch.PET)
		or (class and E.global.unitframe.buffwatch[E.myclass])
		or E.global.unitframe.buffwatch.GLOBAL
	local default = (profile and P.unitframe.filters.buffwatch)
		or (pet and G.unitframe.buffwatch.PET)
		or class and G.unitframe.buffwatch[E.myclass]
		or G.unitframe.buffwatch.GLOBAL
...
local function GetSpellNameRank(id)
	...
	if info and info.includeIDs then
		return format(
			...
			info and info.includeIDs and L["Multiple Ranks"] or rank
		)
	end
...
local function SetSpellList()
	...
	elseif  ]]
		selectedFilter == "AuraBar Colors"
	then
		list = E.global.unitframe.AuraBarColors
	elseif
		selectedFilter == "Aura Indicator (Pet)"
		or selectedFilter == "Aura Indicator (Profile)"
		or selectedFilter == "Aura Indicator (Class)"
		or selectedFilter == "Aura Indicator (Global)"
	then
...
local function AddOrRemoveSpellID(info, value)
	...
	if selectedFilter == "Aura Highlight" then
		...
		elseif not E.global.unitframe.AuraHighlightColors[value] then
			E.global.unitframe.AuraHighlightColors[value] =
				{ enable = true, style = "GLOW", color = { r = 0.8, g = 0, b = 0, a = 0.85 }, ownOnly = false }
		end
	...
	elseif G.unitframe.aurafilters[selectedFilter] and G.unitframe.aurafilters[selectedFilter].spells[value] then
		...
		elseif not E.global.unitframe.aurafilters[selectedFilter].spells[value] then
			E.global.unitframe.aurafilters[selectedFilter].spells[value] =
				{ enable = true, priority = 0, stackThreshold = 0 }
		end
	end
...
local function getSelectedFilter()
	return selectedFilter
end

local function resetSelectedFilter(_, value)
...
local function validateCreateFilter(_, value)
	return not (
		strmatch(value, "^[%s%p]-$")
		or strmatch(value, "^Friendly:")
		or strmatch(value, "^Enemy:")
		or G.unitframe.specialFilters[value]
		or E.global.unitframe.aurafilters[value]
	)
end

local function confirmResetFilter(_, value)
	return value ~= "" and format(L["Reset Filter - %s"], value)
end
...
local function resetFilter(_, value)
	...
	elseif
		value == "Aura Indicator (Pet)"
		or value == "Aura Indicator (Profile)"
		or value == "Aura Indicator (Class)"
		or value == "Aura Indicator (Global)"
	then
		local selectedTable, defaultTable = GetSelectedFilters()
		wipe(selectedTable)
...
Filters.mainOptions.args.createFilter = ACH:Input(
	L["Create Filter"],
...
Filters.mainOptions.args.selectFilter =
	ACH:Select(L["Select Filter"], nil, 2, SetFilterList, nil, nil, getSelectedFilter, resetSelectedFilter)
Filters.mainOptions.args.deleteFilter = ACH:Select(
	L["Delete Filter"],
	...
	DeleteFilterListDisable
)
Filters.mainOptions.args.resetGroup = ACH:Select(
	L["Reset Filter"],
...
Filters.mainOptions.args.filterGroup = ACH:Group(
	function()
		return selectedFilter
	end,
	nil,
	...
	nil,
	function()
		return not selectedFilter
	end
)
Filters.mainOptions.args.filterGroup.inline = true
Filters.mainOptions.args.filterGroup.args.selectSpellheader =
	ACH:Description(L["|cffFF3333Warning:|r Click the arrow on the dropdown box to see a list of spells."], 0, "medium")
Filters.mainOptions.args.filterGroup.args.selectSpell = ACH:Select(
	L["Select Spell"],
	nil,
	1,
	SetSpellList,
	...
	function(_, value)
		selectedSpell = (value ~= "" and value)
	end
)
Filters.mainOptions.args.filterGroup.args.selectSpell.sortByValue = true

Filters.mainOptions.args.filterGroup.args.quickSearch = ACH:Input(
	L["Filter Search"],
...
Filters.mainOptions.args.filterGroup.args.filterType = ACH:Select(
	L["Filter Type"],
	...
	2,
	{ Whitelist = L["Whitelist"], Blacklist = L["Blacklist"] },
	nil,
	nil,
	function()
	...
	function()
		return (
			selectedFilter == "Aura Highlight"
			or selectedFilter == "AuraBar Colors"
			or selectedFilter == "Aura Indicator (Pet)"
			or selectedFilter == "Aura Indicator (Profile)"
			or selectedFilter == "Aura Indicator (Class)"
			or selectedFilter == "Aura Indicator (Global)"
			or selectedFilter == "Whitelist"
			or selectedFilter == "Blacklist"
		) or G.unitframe.aurafilters[selectedFilter]
	end
)
Filters.mainOptions.args.filterGroup.args.removeSpell = ACH:Select(
	L["Remove Spell"],
	nil,
	4,
	SetSpellList,
...
Filters.mainOptions.args.filterGroup.args.addSpell =
	ACH:Input(L["Add SpellID"], L["Add a spell to the filter."], 5, nil, 350, nil, AddOrRemoveSpellID)

Filters.mainOptions.args.auraIndicator = ACH:Group(
	function()
	...
	function()
		return not selectedSpell
			or (
				selectedFilter ~= "Aura Indicator (Pet)"
				and selectedFilter ~= "Aura Indicator (Profile)"
				and selectedFilter ~= "Aura Indicator (Class)"
				and selectedFilter ~= "Aura Indicator (Global)"
			)
	end
)
...
Filters.mainOptions.args.auraIndicator.args.sizeOffset = ACH:Range(
	L["Size Offset"],
	L["This changes the size of the Aura Icon by this value."],
	2,
	{ min = -25, max = 25, step = 1 }
)
Filters.mainOptions.args.auraIndicator.args.style = ACH:Select(
	L["Style"],
	nil,
	3,
	{ timerOnly = L["Timer Only"], coloredIcon = L["Colored Icon"], texturedIcon = L["Textured Icon"] }
)
Filters.mainOptions.args.auraIndicator.args.color = ACH:Color(" ", nil, 4, true, nil, nil, nil, nil, function()
	local spell = GetSelectedSpell()
	if not spell then
		return
	end
...
Filters.mainOptions.args.auraIndicator.args.anyUnit =
	ACH:Toggle(L["Show Aura From Other Players"], nil, 6, nil, nil, 205)
Filters.mainOptions.args.auraIndicator.args.onlyShowMissing = ACH:Toggle(L["Show When Not Active"], nil, 7)
Filters.mainOptions.args.auraIndicator.args.displayText = ACH:Toggle(
	L["Display Text"],
	nil,
	8,
	nil,
...
Filters.mainOptions.args.auraIndicator.args.textThreshold = ACH:Range(
	L["Text Threshold"],
...
Filters.mainOptions.args.auraIndicator.args.positionGroup.args.point =
	ACH:Select(L["Anchor Point"], nil, 5, C.Values.AllPoints)
Filters.mainOptions.args.auraIndicator.args.positionGroup.args.xOffset =
	ACH:Range(L["X-Offset"], nil, 6, { min = -75, max = 75, step = 1 })
Filters.mainOptions.args.auraIndicator.args.positionGroup.args.yOffset =
	ACH:Range(L["Y-Offset"], nil, 7, { min = -75, max = 75, step = 1 })

Filters.mainOptions.args.auraIndicator.args.countGroup = ACH:Group(L["Count"], nil, 20)
Filters.mainOptions.args.auraIndicator.args.countGroup.args.countAnchor =
	ACH:Select(L["Anchor Point"], nil, 1, C.Values.AllPoints)
Filters.mainOptions.args.auraIndicator.args.countGroup.args.countX =
	ACH:Range(L["X-Offset"], nil, 2, { min = -75, max = 75, step = 1 })
Filters.mainOptions.args.auraIndicator.args.countGroup.args.countY =
	ACH:Range(L["Y-Offset"], nil, 3, { min = -75, max = 75, step = 1 })

Filters.mainOptions.args.spellGroup = ACH:Group(
	function()
		return GetSpellNameRank(GetSelectedSpell())
	end,
	nil,
...
Filters.mainOptions.args.spellGroup.args.enable = ACH:Toggle(
	L["Enable"],
	nil,
	0,
	nil,
...
Filters.mainOptions.args.spellGroup.args.style = ACH:Select(
	L["Style"],
	nil,
	1,
	{ GLOW = L["Glow"], FILL = L["Fill"] },
	nil,
	nil,
	nil,
	nil,
	nil,
	function()
		return selectedFilter ~= "Aura Highlight"
	end
)
Filters.mainOptions.args.spellGroup.args.color = ACH:Color(
	L["COLOR"],
	nil,
	2,
	function()
		return selectedFilter ~= "AuraBar Colors"
	end,
	nil,
	nil,
	nil,
	...
	function()
		return (
			selectedFilter ~= "Aura Highlight"
			and selectedFilter ~= "AuraBar Colors"
			and selectedFilter ~= "Aura Indicator (Pet)"
			and selectedFilter ~= "Aura Indicator (Profile)"
			and selectedFilter ~= "Aura Indicator (Class)"
			and selectedFilter ~= "Aura Indicator (Global)"
		)
...
Filters.mainOptions.args.spellGroup.args.removeColor = ACH:Execute(
	L["Restore Defaults"],
	nil,
	3,
	function()
...
Filters.mainOptions.args.spellGroup.args.forDebuffIndicator = ACH:Group(
...
Filters.mainOptions.args.spellGroup.args.forDebuffIndicator.args.priority = ACH:Range(
	L["Priority"],
...
Filters.mainOptions.args.spellGroup.args.forDebuffIndicator.args.stackThreshold = ACH:Range(
	L["Stack Threshold"],
	L["The debuff needs to reach this amount of stacks before it is shown. Set to 0 to always show the debuff."],
	2,
	{ min = 0, max = 99, step = 1 }
)
Filters.mainOptions.args.spellGroup.args.ownOnly = ACH:Toggle(
	L["Casted by Player Only"],
