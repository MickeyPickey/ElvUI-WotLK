...
local function GetFilter(collect, profile)
	local setting = (profile and E.db.nameplates.filters[C.StyleFilterSelected])
		or E.global.nameplates.filters[C.StyleFilterSelected]
		or StyleFallback
...
StyleFilters.removeFilter = ACH:Select(
	L["Delete Filter"],
	...
	function(_, value)
		for profile in pairs(E.data.profiles) do
			if E.data.profiles[profile].nameplates and E.data.profiles[profile].nameplates.filters then
				E.data.profiles[profile].nameplates.filters[value] = nil
			end
		end
		E.global.nameplates.filters[value] = nil
		exportList[value] = nil
		NP:ConfigureAll()
...
do
	...
	local function Import_Get()
		return importText
	end
	local function Import_TextChanged(text)
		if text ~= importText then
			importText = text
		end
	end
	local function Import_Clear()
		label.name = ""
		importText = ""
	end
	local function Import_Decode()
		importText = DecodeLabel(label, importText)
	end
	local function Import_Button()
		if not validateString(nil, importText) then
			return
		end
		label.name = (
			D:Decode(importText) == "styleFilters"
			and D:ImportProfile(importText)
			and L["Profile imported successfully!"]
...
do
	...
	local function Filters_Empty()
		if not next(exportList) then
			StyleFilters.export.args.text.hidden = true
			return true
		end
	end
	...
	StyleFilters.export.args.filters =
		ACH:MultiSelect(L["Filters"], nil, 2, GetFilters, nil, nil, Filters_Get, Filters_Set, nil, nil, true)
	StyleFilters.export.args.exportButton = ACH:Execute(L["Export"], nil, 3, function()
		Export("text")
	end, nil, nil, 120)
	StyleFilters.export.args.exportDecode = ACH:Execute(L["Table"], nil, 4, function()
		Export("luaTable")
	end, nil, nil, 120)
	StyleFilters.export.args.exportPlugin = ACH:Execute(L["Plugin"], nil, 5, function()
		Export("luaPlugin")
	end, nil, nil, 120)
