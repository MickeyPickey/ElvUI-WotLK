...
function E:ConfigMode_AddGroup(layoutName, localizedName)
	if E.ConfigModeLocalizedStrings[layoutName] then
		return
	end
...
function E:CreateMoverPopup()
	...
	nudgeFrame:HookScript("OnShow", function(self)
		self:EnableKeyboard(true)
	end)
	nudgeFrame:HookScript("OnHide", function(self)
...
function E:Config_StopMoving()
	...
	if status then
		...
		E.global.general.AceGUI.width, E.global.general.AceGUI.height =
			E:Round(frame:GetWidth(), 2), E:Round(frame:GetHeight(), 2)
		E:Config_UpdateLeftScroller(frame)
	end
end
...
function E:Config_SetButtonColor(btn, disabled)
	...
	if disabled then
		...
		if btn.SetBackdropColor then
			btn:SetBackdropColor(1, 0.82, 0, 0.4)
...
function E:Config_UpdateLeftButtons()
	local frame = E:Config_GetWindow()
	if not (frame and frame.leftHolder) then
		return
	end
...
function E:Config_ContentPlacement(frame, content, unskinned, statusShown)
	...
	content:Point(
		...
		frame,
		"TOPLEFT",
		unskinned and 13 or 7,
		-(frame.bottomHolder:GetHeight() + (unskinned and 46 or 41))
	)
	content:Point(
		"BOTTOMRIGHT",
		frame,
		"BOTTOMRIGHT",
		-(unskinned and 18 or 8),
		(statusShown and (unskinned and 32 or 25)) or (unskinned and 12) or 2
	)
end

function E:Config_SetStatusText(text)
	if not ConfigLogoTop or not self.parent then
		return
	end
	...
	if frame and frame.bottomHolder and not ConfigLogoTop then
		...
		if statusParent then
			...
			statusParent:Point(
				...
				frame.leftHolder,
				"BOTTOMRIGHT",
				unskinned and 11 or 1,
				unskinned and 38 or 22
			)
...
function E:Config_CreateBottomButtons(frame, unskinned)
	...
	for index, info in ipairs({
		...
		if info.var == "RepositionWindow" then
			...
			texture:SetTexture(
				unskinned and [[Interface\ChatFrame\UI-ChatIcon-Maximize-Up]] or E.Media.Textures.Resize2
			)
		...
		elseif info.editBox then
			...
			element =
				E:Config_CreateFrame(info, frame, unskinned, "Button", nil, frame.bottomHolder, "UIPanelButtonTemplate")
		end
			...
			if pageCount > 1 then
				...
				for i = 1, pageCount do
					if i == 1 then
						...
						subNode = (
							mainSelStr
							and msgStr:match(mainSelStr .. E:EscapeString(pages[i]) .. "$")
