...
function E:GetFormattedText(style, min, max, dec, short)
	...
	then
		...
		if style == "DEFICIT" then
			...
			return (
				deficit > 0 and format(useStyle, short and E:ShortValue(deficit, dec) or BreakUpLargeNumbers(deficit))
			) or ""
		elseif style == "CURRENT_MAX" then
			return format(
				useStyle,
				short and E:ShortValue(min, dec) or BreakUpLargeNumbers(min),
				short and E:ShortValue(max, dec) or BreakUpLargeNumbers(max)
			)
...
function E:FormatLargeNumber(amount, seperator)
	...
	for i = 4, len, 3 do
		num = seperator .. strsub(amount, -(i - 1), -(i - 3)) .. num
	end
