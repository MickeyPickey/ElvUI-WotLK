...
do
	function M:COMBAT_LOG_EVENT_UNFILTERED(
		...
		local announce = spellName
			and (destGUID ~= E.myguid)
			and (sourceGUID == E.myguid or sourceGUID == UnitGUID("pet"))
			and strmatch(event, "_INTERRUPT")
			...
			if STATUS == "GUILD_REPAIR_FAILED" then
				...
				E:Print(
					L["Your items have been repaired using guild bank funds for: "]
						.. E:FormatMoney(COST, B.db.moneyFormat, not B.db.moneyCoins)
				)
			end
...
function M:AutoInvite(_, leaderName)
	...
	if _G.MiniMapLFGFrame:IsShown() then
		return
	end
