...
local LEVEL2 = strlower(
	_G.TOOLTIP_UNIT_LEVEL_CLASS:gsub("^%%2$s%s?(.-)%s?%%1$s", "%1"):gsub("^%-?г?о?%s?", ""):gsub("%s?%%s%s?%-?", "")
)
...
function TT:IsModKeyDown(db)
	...
	return k == "SHOW"
		or (
			(k == "SHIFT" and IsShiftKeyDown())
			or (k == "CTRL" and IsControlKeyDown())
			or (k == "ALT" and IsAltKeyDown())
		)
end
...
function TT:GameTooltip_SetDefaultAnchor(tt, parent)
	if not E.private.tooltip.enable or not TT.db.visibility or tt:GetAnchorType() ~= "ANCHOR_NONE" then
		return
	elseif
		(InCombatLockdown() and not TT:IsModKeyDown(TT.db.visibility.combatOverride))
		or (
			not AB.KeyBinder.active
			and not TT:IsModKeyDown(TT.db.visibility.actionbars)
			and AB.handledbuttons[tt:GetOwner()]
		)
	then
	...
	if parent then
		if TT.db.cursorAnchor then
			local anchor = (
				TT.db.cursorAnchorType == "ANCHOR_CURSOR_LEFT" and "ANCHOR_CURSOR_RIGHT" or TT.db.cursorAnchorType
			)
			local pointX = (
				(TT.db.cursorAnchorType == "ANCHOR_CURSOR_LEFT" and -128 + TT.db.cursorAnchorX or 0)
				+ TT.db.cursorAnchorX
			)
	...
	if
		anchor == nil
		or anchor == B.BagFrame
		or anchor == RightChatPanel
		or anchor == TooltipMover
		or anchor == _G.GameTooltipDefaultContainer
		or anchor == UIParent
...
function TT:RemoveTrashLines(tt)
	...
	if not (info and info.lines[3]) then
		return
	end
...
function TT:GetLevelLine(tt, offset, raw)
	local info = tt:GetTooltipData()
	if not (info and info.lines[offset]) then
		return
	end
...
function TT:SetUnitText(tt, unit, isPlayerUnit)
	...
	if isPlayerUnit then
		...
		if guildName then
			local text = TT.db.guildRanks and format("<|cff00ff10%s|r> [|cff00ff10%s|r]", guildName, guildRankName)
				or format("<|cff00ff10%s|r>", guildName)
		...
		if levelLine then
			...
			if
				...
				or creatureClassification == "elite"
				or creatureClassification == "rareelite"
				or creatureClassification == "worldboss"
			then
				classificationString =
					format("%s %s|r", E:CallTag("classificationcolor", unit), E:CallTag("classification", unit))
			end
	...
	local nameColor = unitReaction
			and ((TT.db.useCustomFactionColors and TT.db.factionColors[unitReaction]) or _G.FACTION_BAR_COLORS[unitReaction])
		or PRIEST_COLOR
...
function TT:AddInspectInfo(tt, unit, numTries, r, g, b)
	if
		...
		or not CheckInteractDistance(unit, 4)
	then
		return
	end
...
function TT:AddTargetInfo(tt, unit)
	...
	if unit ~= "player" and UnitExists(unitTarget) then
		...
		if UnitIsPlayer(unitTarget) and not UnitHasVehicleUI(unitTarget) then
			...
			targetColor = (TT.db.useCustomFactionColors and TT.db.factionColors[reaction])
...
function TT:GameTooltip_OnTooltipSetUnit(data)
	if self ~= GameTooltip or not TT.db.visibility then
		return
	end
...
function TT:GameTooltipStatusBar_OnValueChanged(tt, value)
	if not value or not tt.text or not TT.db.healthBar.text then
		return
	end
...
function TT:GameTooltip_OnTooltipSetItem(data)
	if
		(self ~= GameTooltip and self ~= _G.ShoppingTooltip1 and self ~= _G.ShoppingTooltip2) or not TT.db.visibility
	then
		return
	end
	...
	if
		ownerName
		and (strfind(ownerName, "ElvUI_Container") or strfind(ownerName, "ElvUI_BankContainer"))
		and not TT:IsModKeyDown(TT.db.visibility.bags)
	then
		self:Hide()
		return
	end
...
function TT:GameTooltip_AddQuestRewardsToTooltip(tt, questID)
	if not (tt and questID and tt.progressBar) then
		return
	end
...
function TT:GameTooltip_ShowProgressBar(tt)
	if not tt or not tt.progressBarPool then
		return
	end
...
function TT:SetStyle(tt)
	if not tt or (tt == E.ScanTooltip) then
		return
	end
...
function TT:SetUnitAura(tt, ...)
	...
	if TT:IsModKeyDown() then
		if source then
			...
			tt:AddDoubleLine(
				format(IDLine, _G.ID, spellID),
				format("|c%s%s|r", color.colorStr, UnitName(source) or UNKNOWN)
			)
		else
...
function TT:GameTooltip_OnTooltipSetSpell(data)
	if (self ~= GameTooltip and self ~= E.SpellBookTooltip) or not TT:IsModKeyDown() then
		return
	end
...
function TT:SetItemRef(link)
	if IsModifierKeyDown() or not (link and strfind(link, "^spell:")) then
		return
	end
...
function TT:SetTooltipFonts()
	...
	_G.GameTooltipHeaderText:FontTemplate(
		LSM:Fetch("font", TT.db.headerFont),
		TT.db.headerFontSize,
		TT.db.headerFontOutline
	)
...
function TT:Initialize()
	...
	statusText:FontTemplate(
		LSM:Fetch("font", TT.db.healthBar.font),
		TT.db.healthBar.fontSize,
		TT.db.healthBar.fontOutline
	)
