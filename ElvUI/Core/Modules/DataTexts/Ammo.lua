...
local function OnEnter()
				...
				if info and info.itemID and not itemCount[info.itemID] then
					...
					if equipLoc == "INVTYPE_AMMO" or equipLoc == "INVTYPE_THROWN" then
						DT.tooltip:AddDoubleLine(
							strjoin("", format(iconString, texture), " ", name),
							count,
							E:GetItemQualityColor(quality)
						)
	...
	for i = 1, NUM_BAG_SLOTS do
		...
		if itemID then
			...
			if
				(itemSubType == QUIVER or itemSubType == POUCH or itemSubType == SOULBAG)
				or (
					itemType == "Container"
					and (itemSubType == QUIVER or itemSubType == POUCH or itemSubType == SOULBAG)
				)
			then
