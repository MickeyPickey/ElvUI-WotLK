...
local function AddInfo(id)
	...
	if name then
		...
		DT.tooltip:AddDoubleLine(
			...
			format(textRight, BreakUpLargeNumbers(info.quantity)),
			1,
			1,
			1,
			1,
...
local function AddHeader(id, addLine)
	if (not db.headers) or shownHeaders[id] then
		return
	end
		...
		for i = 1, 3 do
			...
			if info and info.quantity then
				iconString = match(info and info.iconFileID or "", E.myfaction) ~= nil
					...
					or iconString
				displayString = (i > 1 and displayString .. " " or "")
					.. format("%s %s", format(iconString, info.iconFileID), E:ShortValue(info.quantity))
			end
		end
...
DT:RegisterDatatext("Currencies", nil, {
	"PLAYER_MONEY",
...
}, OnEvent, nil, OnClick, OnEnter, nil, _G.CURRENCY, nil, ApplySettings)
