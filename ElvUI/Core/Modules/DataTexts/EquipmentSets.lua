...
local function OnEnter()
	...
	for _, set in pairs(eqSets) do
		DT.tooltip:AddLine(set.text, set.isEquipped and 0.2 or 1, set.isEquipped and 1 or 0.2, 0.2)
	end
...
local function OnEvent(self, event)
	...
	for i = 1, numSets do
		...
		then
			tinsert(eqSets, {
				text = format("|T%s:20:20:0:0:64:64:4:60:4:60|t  %s", iconFileID, name),
				checked = isEquipped,
				func = function()
...
DT:RegisterDatatext(
	"Equipment Sets",
	nil,
	{ "EQUIPMENT_SETS_CHANGED", "PLAYER_EQUIPMENT_CHANGED", "EQUIPMENT_SWAP_FINISHED" },
	OnEvent,
	nil,
	OnClick,
	OnEnter,
