...
local function OnEnter()
	...
	if _G.WintergraspTimer.canQueue and startTime ~= 0 then
		...
		DT.tooltip:AddDoubleLine(
			format(formatBattleGroundInfo, L["Wintergrasp"]),
			startTime,
			1,
			1,
			1,
	...
	for i = 1, GetNumSavedInstances() do
		...
		if name and (locked or extended) then
			...
			if isRaid or isDungeon then
				...
				local buttonImg = instanceIconByName[name]
						and format("|T%s:20:20:0:0:96:96:0:64:0:64|t ", instanceIconByName[name])
					or ""

				tinsert(
					lockedInstances[isRaid and "raids" or "dungeons"],
					{ sortName, difficultyLetter, buttonImg, info }
				)
			end
		end
	end
	...
	if next(lockedInstances.raids) then
		...
		for _, info in next, lockedInstances.raids do
			...
			DT.tooltip:AddDoubleLine(
				format(lockoutInfoFormatNoEnc, buttonImg, maxPlayers, difficultyLetter, name),
				ToTime(reset),
				1,
				1,
				1,
	...
	if next(lockedInstances.dungeons) then
		...
		for _, info in next, lockedInstances.dungeons do
			local difficultyLetter, buttonImg = info[2], info[3]
			local name, _, reset, _, _, extended, _, _, maxPlayers, _, numEncounters, encounterProgress =
				unpack(info[4])
	...
	DT.tooltip:AddDoubleLine(
		db.localTime and TIMEMANAGER_TOOLTIP_REALMTIME or TIMEMANAGER_TOOLTIP_LOCALTIME,
		format(displayFormats[AmPm == -1 and "eu_nocolor" or "na_nocolor"], Hr, Min, Sec, APM[AmPm]),
		1,
		1,
		1,
		lockoutColorNormal.r,
...
DT:RegisterDatatext(
	"Time",
	nil,
	{ "UPDATE_INSTANCE_INFO", "LOADING_SCREEN_ENABLED" },
	OnEvent,
	OnUpdate,
	OnClick,
	OnEnter,
