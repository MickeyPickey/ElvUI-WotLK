...
local function OnEvent(self)
	...
	if db.NoLabel then
		...
		self.text:SetFormattedText(
			displayString,
			db.Label ~= "" and db.Label or STAT_HIT_CHANCE .. ": ",
			hitPercent + hitPercentFromTalents
		)
	end
end
...
local function OnEnter()
	...
	DT.tooltip:AddLine(
		format(
			ratingIndex == CR_HIT_MELEE and CR_HIT_MELEE_TOOLTIP or CR_HIT_RANGED_TOOLTIP,
			E.mylevel,
			hitPercent,
			GetCombatRating(CR_ARMOR_PENETRATION),
			GetArmorPenetration()
		)
	)
