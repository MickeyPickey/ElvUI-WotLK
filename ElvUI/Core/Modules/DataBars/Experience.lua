...
function DB:ExperienceBar_QuestXP()
	...
	if bar.db.showQuestXP and QuestLogXP > 0 then
		...
		bar.Quest:SetStatusBarColor(
			DB.db.colors.quest.r,
			DB.db.colors.quest.g,
			DB.db.colors.quest.b,
			DB.db.colors.quest.a
		)
		RestedQuestLayering()
		bar.Quest:Show()
	else
...
function DB:ExperienceBar_OnEnter()
	...
	if CurrentXP then
		...
		GameTooltip:AddDoubleLine(
			L["XP:"],
			format(" %s / %s (%.2f%%)", E:ShortValue(CurrentXP), E:ShortValue(XPToLevel), PercentXP),
			1,
			1,
			1
		)
	...
	if RemainXP then
		GameTooltip:AddDoubleLine(
			L["Remaining:"],
			format(" %s (%.2f%% - %.2f " .. L["Bars"] .. ")", RemainXP, RemainTotal, RemainBars),
			1,
			1,
			1
		)
	...
	if QuestLogXP > 0 then
		GameTooltip:AddDoubleLine(
			L["Quest Log XP:"],
			format(" %d (%.2f%%)", QuestLogXP, (QuestLogXP / XPToLevel) * 100),
			1,
			1,
			1
		)
...
function DB:ExperienceBar_OnClick(button)
	if not _G.XPRM then
		return
	end -- Warmane Experience Dropdown
...
function DB:ExperienceBar()
	local Experience = DB:CreateBar(
		"ElvUI_ExperienceBar",
		"Experience",
		DB.ExperienceBar_Update,
		DB.ExperienceBar_OnEnter,
		DB.ExperienceBar_OnClick,
		{ "BOTTOM", E.UIParent, "BOTTOM", 0, 43 }
	)
