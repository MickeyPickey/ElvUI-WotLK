...
function DB:PetExperienceBar_OnEnter()
	...
	GameTooltip:AddDoubleLine(
		L["Remaining:"],
		format(" %d (%d%% - %d " .. L["Bars"] .. ")", max - cur, (max - cur) / max * 100, 20 * (max - cur) / max),
		1,
		1,
		1
	)
...
function DB:PetExperienceBar()
	local PetExperience = DB:CreateBar(
		"ElvUI_PetExperienceBar",
		"PetExperience",
		DB.PetExperienceBar_Update,
		DB.PetExperienceBar_OnEnter,
		DB.PetExperienceBar_OnClick,
		{ "LEFT", _G.LeftChatPanel, "RIGHT", -E.Border + E.Spacing * 3, 0 }
	)
	DB:CreateBarBubbles(PetExperience)

	PetExperience.ShouldHide = function()
		return not HasPetUI()
			or (HasPetUI() and DB.db.petExperience.hideAtMaxLevel and not DB:PetExperienceBar_ShouldBeVisible())
	end
