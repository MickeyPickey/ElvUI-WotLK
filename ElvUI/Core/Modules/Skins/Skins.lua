...
function S:HandleInsetFrame(frame)
	...
	if frame.InsetBorderTop then
		frame.InsetBorderTop:Hide()
	end
	if frame.InsetBorderTopLeft then
		frame.InsetBorderTopLeft:Hide()
	end
	if frame.InsetBorderTopRight then
		frame.InsetBorderTopRight:Hide()
	end

	if frame.InsetBorderBottom then
		frame.InsetBorderBottom:Hide()
	end
	if frame.InsetBorderBottomLeft then
		frame.InsetBorderBottomLeft:Hide()
	end
	if frame.InsetBorderBottomRight then
		frame.InsetBorderBottomRight:Hide()
	end

	if frame.InsetBorderLeft then
		frame.InsetBorderLeft:Hide()
	end
	if frame.InsetBorderRight then
...
do -- DropDownMenu library support
	...
	function S:SkinLibDropDownMenu(prefix)
		if S[prefix .. "_UIDropDownMenuSkinned"] then
			return
		end
...
do -- WIM replaces Blizzard globals we need to rehook
	...
	function S:SkinDropDownMenu(prefix, textX, textY)
		if S.DropDownMenu_Hooks[prefix] then
			return
		end
		S.DropDownMenu_Hooks[prefix] = true

		hooksecurefunc("UIDropDownMenu_CreateFrames", function(level, index)
			S:DropDownMenu_CreateFrames(prefix, level, index)
		end)
		hooksecurefunc("ToggleDropDownMenu", function(level)
			S:DropDownMenu_Toggle(prefix, level, textX, textY)
		end)
	end
end
...
function S:HandleButton(
	...
	if button.Icon then
		...
		if
			Texture and (type(Texture) == "string" and strfind(Texture, [[Interface\ChatFrame\ChatFrameExpandArrow]]))
		then
...
function S:HandleSearchBox(frame, unskinned)
	...
	frame.clearButton:SetScript("OnMouseDown", function(self)
		if self:IsEnabled() then
			self.texture:Point("CENTER", 1, -1)
		end
	end)
	frame.clearButton:SetScript("OnMouseUp", function(self)
		self.texture:Point("CENTER")
	end)
	...
	if not unskinned or frame.backdrop then
		return
	end
...
function S:HandleStatusBar(frame, color, template)
	...
	frame:SetStatusBarColor(unpack(color or { 0.01, 0.39, 0.1 }))
...
do
	...
	local function checkNormalTexture(checkbox, texture)
		if texture ~= E.ClearTexture then
			checkbox:SetNormalTexture(E.ClearTexture)
		end
	end
	local function checkPushedTexture(checkbox, texture)
		if texture ~= E.ClearTexture then
			checkbox:SetPushedTexture(E.ClearTexture)
		end
	end
	local function checkHighlightTexture(checkbox, texture)
		if texture ~= E.ClearTexture then
			checkbox:SetHighlightTexture(E.ClearTexture)
		end
	end
	local function checkCheckedTexture(checkbox, texture)
		if texture == E.Media.Textures.Melli or texture == check then
			return
		end
	...
	local function checkOnDisable(checkbox)
		...
		checkbox:SetDisabledTexture(
			checkbox:GetChecked() and (E.private.skins.checkBoxSkin and E.Media.Textures.Melli or disabled) or ""
		)
	end
...
do
	...
	local function buttonNormalTexture(frame, texture)
		if texture ~= E.ClearTexture then
			frame:SetNormalTexture(E.ClearTexture)
		end
	end
	local function buttonPushedTexture(frame, texture)
		if texture ~= E.ClearTexture then
			frame:SetPushedTexture(E.ClearTexture)
		end
	end
	local function buttonDisabledTexture(frame, texture)
		if texture ~= E.ClearTexture then
			frame:SetDisabledTexture(E.ClearTexture)
		end
	end
	local function buttonHighlightTexture(frame, texture)
		if texture ~= E.ClearTexture then
			frame:SetHighlightTexture(E.ClearTexture)
		end
	end
...
do
	...
	function S:HandleIconBorder(border, backdrop)
		if not border then
			return
		end -- IconBorder only exists on patched (HD) clients
...
do
	...
	function S:HandleNextPrevButton(btn, arrowDir, color, noBackdrop, stripTexts, frameLevel)
		...
		if not arrowDir then
			...
			if ButtonName then
				if
					strfind(ButtonName, "left")
					or strfind(ButtonName, "prev")
					or strfind(ButtonName, "decrement")
					or strfind(ButtonName, "backward")
					or strfind(ButtonName, "back")
				then
					arrowDir = "left"
				elseif
					strfind(ButtonName, "right")
					or strfind(ButtonName, "next")
					or strfind(ButtonName, "increment")
					or strfind(ButtonName, "forward")
				then
					arrowDir = "right"
				elseif
					strfind(ButtonName, "scrollup")
					or strfind(ButtonName, "upbutton")
					or strfind(ButtonName, "top")
					or strfind(ButtonName, "asc")
					or strfind(ButtonName, "home")
					or strfind(ButtonName, "maximize")
				then
					arrowDir = "up"
				end
			end
...
do -- Handle collapse
	...
	function S:HandleCollapseTexture(button, syncPushed, ignorePushed)
		if button.collapsedSkinned then
			return
		end
