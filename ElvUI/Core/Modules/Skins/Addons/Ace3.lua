...
function S:Ace3_CheckBoxIsEnable(widget)
	...
	if text and S.Ace3_EnableMatch then
		return strmatch(text, S.Ace3_EnableMatch)
	end
end
...
function S:Ace3_CheckBoxSetDisabled(disabled)
	if S:Ace3_CheckBoxIsEnable(self) then
		...
		self:SetLabel(
			(tristateOrDisabled and S.Ace3_L.Enable) or (self.checked and S.Ace3_EnableOn) or S.Ace3_EnableOff
		)
	end
end
...
function S:Ace3_RefreshTree(scrollToSelection)
	...
	if not E.private.skins.ace3Enable then
		return
	end
...
function S:HookAce3(lib, minor, early, registering) -- lib: AceGUI
	...
	if earlyContainer then
		lib.RegisterAsContainer = earlyContainer
	end
	if earlyWidget then
		lib.RegisterAsWidget = earlyWidget
	end
end
	...
	function S:LibStub_NewLib(major, requestedMinor)
		...
		if Libraries[n] then
			...
			elseif n == "LibDropDownMenu" then
				...
				local prefix = (n == "NoTaint_UIDropDownMenu" and "Lib")
					or (n == "LibUIDropDownMenuQuestie" and "LQuestie")
					or (major == "LibUIDropDownMenu-4.0" and "L4" or major == "LibUIDropDownMenu-3.0" and "L3")
