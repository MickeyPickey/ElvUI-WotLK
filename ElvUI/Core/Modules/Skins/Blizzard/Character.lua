...
local function HandleTabs()
	...
	for index, tab in
		next,
		{
			...
			hasPetUI and _G.CharacterFrameTab2 or nil,
	...
	do
		...
		if index == 1 then
			tab:Point(
				...
				"BOTTOMLEFT",
				10,
				IsAddOnLoaded("ElvUI_Enhanced") and E.private.enhanced.character.enable and not E:IsHDPatch() and 80
					or 78
			) -- check if using ElvUI_Enhanced
		else
...
local function UpdateCurrencySkins()
	...
	for _, button in next, TokenFrameContainer.buttons do
		...
		if button.categoryLeft then
			button.categoryLeft:Kill()
		end
		if button.categoryRight then
			button.categoryRight:Kill()
		end
		if button.categoryMiddle then
...
S:AddCallback("Skin_Character", function()
	...
	hooksecurefunc("PaperDollFrameItemFlyout_DisplayButton", function(button)
		...
		if not button.location or button.location >= _G.PDFITEMFLYOUT_FIRST_SPECIAL_LOCATION then
			return
		end
	...
	for i, button in ipairs(_G.GearManagerDialogPopup.buttons) do
		...
		if i > 1 then
			...
			if lastPos == math.floor(lastPos) then
				button:SetPoint(
					"TOPLEFT",
					_G.GearManagerDialogPopup.buttons[i - _G.NUM_GEARSET_ICONS_PER_ROW],
					"BOTTOMLEFT",
					0,
					-7
				)
	...
	_G.GearManagerDialogPopupScrollFrameScrollBar:Point(
		"TOPLEFT",
		_G.GearManagerDialogPopupScrollFrame,
		"TOPRIGHT",
		3,
		-19
	)
	...
	_G.SkillDetailStatusBarUnlearnButton:HookScript("OnEnter", function(btn)
		btn.Texture:SetVertexColor(1, 1, 1)
	end)
	_G.SkillDetailStatusBarUnlearnButton:HookScript("OnLeave", function(btn)
